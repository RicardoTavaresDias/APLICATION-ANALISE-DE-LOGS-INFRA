...
output.innerHTML = ""
    ...
    const taskTimeFrom = document.getElementById('task-time-from').value;
    const taskTimeTo = document.getElementById('task-time-to').value;
    ...
    try {
        ...
        if (response.ok) addMessage('task-output', '<p class="sucess">✅ Logs Concluido!</p>');
        else addMessage('task-output', '<p>É necessário informar a data de início e a data de término para continuar.</p>', true);
    } catch (error) {
